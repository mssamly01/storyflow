...
export function buildFinalVisualPrompt(params: {
  ...
  return compact([
    ...
    source.visualMoment ? `visual moment: ${source.visualMoment}` : '',
    source.mainAction || source.action ? `visible action: ${source.mainAction || source.action}` : '',
    source.interaction ? `interaction: ${source.interaction}` : '',
    source.interactionTarget?.length ? `interaction targets: ${source.interactionTarget.map((item) => `${item.actor} -> ${item.target}: ${item.interaction}`).join(' | ')}` : '',
    source.posture ? `posture: ${source.posture}` : '',
    source.environmentDetails ? `beat environment details: ${source.environmentDetails}` : '',
